  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        ".about-socials",
        { opacity: 0, y: 14 },
        { opacity: 1, y: 0, duration: 0.5, ease: "expo.out", delay: 0.18 }
      );
      gsap.fromTo(
        ".about-bio p",
        { opacity: 0, y: 16 },
        { opacity: 1, y: 0, duration: 0.6, ease: "expo.out", stagger: 0.1, delay: 0.22 }
      );
      gsap.fromTo(
        ".about-meta > div",
        { opacity: 0, y: 12 },
      ...
      <div className="mt-auto grid grid-cols-1 gap-10 pt-10 md:grid-cols-2 md:gap-16">
        <div className="about-bio flex flex-col gap-4">
          ...
          <p className="font-body text-sm leading-relaxed text-foreground sm:text-base">
            I&rsquo;m Deep, a software engineer based in Kolkata, India. My focus is
          ...
          <p className="font-body text-sm leading-relaxed text-muted sm:text-base">
            ...
            mobile applications — picking whichever layer gets the product where it
        ...
        <div className="about-meta grid grid-cols-2 gap-x-8 gap-y-8 self-start">
          <div>
            ...
            <ul className="flex flex-col gap-0.5 font-body text-sm text-foreground">
              {["React, Next.js", "TypeScript", "React Native", "Node.js"].map((t) => (
                <li key={t}>{t}</li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-0.5 font-body text-sm text-foreground">
              {["Frontend Architecture", "Full-stack Products", "Mobile (React Native)", "Agentic and Gen AI"].map((f) => (
                <li key={f}>{f}</li>
              ))}
            </ul>
          </div>
